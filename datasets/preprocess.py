...
def mapping(dict_dataset_all, multiple_datsets_on=False,args=None):
    """
    ...
    local_map = {}
    for dataset in dict_dataset_all.keys():
        local_map[dataset] = {}
        dataset_cur_path = dict_dataset_all[dataset]['current_dataset_path']
       # print(" dataset_cur_path  ", dataset_cur_path)
        local_max_nodes = 0
        local_max_edges= 0
        local_max_degree = 0
        local_min_nodes = float('inf')
        local_min_edges = float('inf')
        for filename in tqdm(os.listdir(dataset_cur_path)):
            if filename.endswith(".dat"):
                path =dataset_cur_path
                f = open(path + filename, 'rb')
                G = pickle.load(f)
                f.close()
                local_max_nodes = max(local_max_nodes, len(G.nodes()))
                local_min_nodes = min(local_min_nodes, len(G.nodes()))        
                ...
                for _, data in G.nodes.data():
                    if data['label'] not in node_forward:
                        ...
                        node_count += 1
                local_max_edges = max(local_max_edges, len(G.edges()))
                local_min_edges = min(local_min_edges, len(G.edges()))
                ...
                for _, _, data in G.edges.data():
                    if data['label'] not in edge_forward:
                        ...
                        edge_count += 1
                local_max_degree = max(local_max_degree,max([d for n, d in G.degree()]))
                max_degree = max(max_degree, max([d for n, d in G.degree()]))
        local_map[dataset]['max_nodes'] = local_max_nodes
        local_map[dataset]['min_nodes'] = local_min_nodes
        local_map[dataset]['max_edges'] = local_max_edges
        local_map[dataset]['min_edges'] = local_min_edges
        local_map[dataset]['max_degree'] = local_max_degree
        
    for dataset in dict_dataset_all.keys():
        dest = dict_dataset_all[dataset]['current_dataset_path'] +'map.dict'

        # if (multiple_datsets_on):
        #     dest = dict_dataset_all[dataset]['current_dataset_path'] + 'multiple_dataset_map.dict'
    ...
    if args is not None:
        ...
        f = open(args.current_model_save_path+"/map.dict","wb")
        pickle.dump(feature_map, f)
        f.close()
