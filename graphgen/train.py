...
def evaluate_loss(args, model, data, feature_map, criterion_cur=None):
    ...
    if args.loss_type == 'BCE':
        ...
        x_pred = pack_padded_sequence(x_pred, (x_len + 1).cpu(), batch_first=True)
        ...
        #
        if(criterion_cur is not None):
            loss = criterion_cur(x_pred, x_target,x_len )
    ...
    for _ in range(eval_args.count // eval_args.batch_size):
        ...
        for i in range(eval_args.batch_size):
            ...
            for j in range(eval_args.max_num_edges):
                ...
                if pred[i, j, 0] == eval_args.max_num_node or pred[i, j, 1] == eval_args.max_num_node                        or pred[i, j, 2] == len_node_vec - 1 or pred[i, j, 3] == len_edge_vec - 1 \
